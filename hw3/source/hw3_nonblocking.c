...
int main(int argc, char **argv) {
    ...
    MPI_Irecv(game_board[0], N, MPI_INT, prev, 0, MPI_COMM_WORLD, &reqs[0]);
    MPI_Irecv(game_board[M-1], N, MPI_INT, next, 1, MPI_COMM_WORLD, &reqs[1]);

    MPI_Isend(game_board[M-2], N, MPI_INT, next, 0, MPI_COMM_WORLD, &reqs[2]);
    MPI_Isend(game_board[1], N, MPI_INT, prev, 1, MPI_COMM_WORLD, &reqs[3]);
